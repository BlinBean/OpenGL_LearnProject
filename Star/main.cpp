...
//#pragma endregion
	...
	//挖顶点
	glVertexAttribPointer(0,3,GL_FLOAT,GL_FALSE,6*sizeof(GL_FLOAT),(void*)0);//解释顶点（顶点属性）//设置存储在0号，顶点属性数量<1,1,1>,数据类型，是否归一化，数据间的间隔,偏移量
	//每抓3个为一个顶点
	glEnableVertexAttribArray(0);//启用顶点属性//0号属性（vertex）//1号color
	
	//挖颜色
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(GL_FLOAT), (void*)(3*sizeof(float)));//解释顶点（顶点属性）//设置存储在1号，顶点属性数量<1,1,1>,数据类型，是否归一化，数据间的间隔,偏移量
//每抓3个为一个顶点
	glEnableVertexAttribArray(1);//启用顶点属性//0号属性（vertex）//1号color
